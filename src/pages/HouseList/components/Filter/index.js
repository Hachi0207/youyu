...
export default class Filter extends Component {
  ...
  componentDidMount() {
    // 获取到body
    // 优化：获取body元素，当条件筛选框被点击时，设置body样式overflow：hidden，让页面不滚动
  ...
  async getFiltersData() {
    // 获取当前定位城市id
    const { value } = JSON.parse(localStorage.getItem('y_city'))
  ...
  onTitleClick = (type) => {
    ...
    // 创建新的标题选中状态对象(因为不能直接修改state里面的数据)
  ...
  renderFilterPicker() {
    ...
    return (
      <FilterPicker
        // 设置key是为了来回点击筛选按钮时，能因为key不同销毁重新创建组件，这样state值才能被初始化
        key={openType}
